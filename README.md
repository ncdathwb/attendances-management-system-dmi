...
> Hệ thống quản lý chấm công thông minh với tính năng đa vai trò, chữ ký số, quản lý nghỉ phép và báo cáo chi tiết
>
> **Phiên bản**: 2.0 - Đã được tối ưu hóa toàn diện với UI/UX thống nhất và logic phê duyệt cải tiến
>
...
* **Phạm vi**:
  ...
  - Phê duyệt chấm công theo phân quyền phân cấp (Team Leader → Manager → Admin)
  - Quản lý nghỉ phép với workflow phê duyệt thống nhất
  - Đổi mật khẩu cá nhân trong cài đặt
  - Báo cáo và xuất dữ liệu
  - Quản lý người dùng và phòng ban
  - UI/UX thống nhất với CSS variables và toast notifications
* **Ngoài phạm vi**: Tích hợp với hệ thống lương

## ✨ Tính năng mới trong phiên bản 2.0

### 🎨 UI/UX được tối ưu toàn diện
- **CSS Variables thống nhất**: Màu sắc, font chữ, khoảng cách đồng bộ
- **Toast notifications**: Thông báo thống nhất về màu sắc, vị trí, hiệu ứng
- **Responsive design**: Tương thích mọi thiết bị
- **Font system**: Roboto font family cho toàn bộ hệ thống

### 🔐 Bảo mật và quản lý tài khoản
- **Đổi mật khẩu cá nhân**: Tính năng đổi mật khẩu trong phần Cài đặt
- **Password validation**: Kiểm tra mật khẩu mạnh (tối thiểu 6 ký tự)
- **Toggle password visibility**: Hiển thị/ẩn mật khẩu khi nhập

### 📋 Logic phê duyệt nghỉ phép cải tiến
- **Workflow phân cấp**: Thống nhất với logic phê duyệt chấm công
  - Employee → Team Leader → Manager → Admin
- **Trạng thái chi tiết**: pending → pending_manager → pending_admin → approved/rejected
- **Audit trail**: Theo dõi người phê duyệt và thời gian phê duyệt

### 🧹 Code quality và performance
- **Clean code**: Loại bỏ debug prints, comments không cần thiết
- **Optimized CSS**: Centralized stylesheet với variables
- **Database migration**: Cập nhật schema cho tính năng mới
...
# 4) Copy biến môi trường (tùy chọn)
cp .env.example .env
...
```bash
# Chạy system test tự động
python test_system.py

# Test với URL tùy chỉnh
python test_system.py http://localhost:8000
```
...
* **System test**: Kiểm tra server, login, dashboard, CSS loading
* **Manual test**: Kiểm tra UI/UX, chức năng đổi mật khẩu, phê duyệt nghỉ phép
* **Coverage target**: ≥ 80%
