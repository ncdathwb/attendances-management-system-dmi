...
// ===== Global UI Blocking Overlay for role switching and heavy ops =====
(function setupGlobalBlockingOverlay(){
    // Nếu overlay toàn cục đã được khởi tạo hoặc API đã tồn tại thì bỏ qua
    if (window.__GLOBAL_OVERLAY_INITED__ || typeof window.__showBlocking === 'function') {
        return;
    }
    let overlayEl = null;
    let styleEl = null;
    ...
    function ensureStyle(){
        ...
        styleEl.textContent = `
        .global-blocking-overlay{position:fixed;inset:0;background:rgba(15,23,42,0.45);backdrop-filter:saturate(120%) blur(2px);z-index:2147483000;display:flex;align-items:center;justify-content:center}
        .global-blocking-card{background:#0b1220;color:#e6edf3;border:1px solid rgba(255,255,255,0.08);border-radius:12px;box-shadow:0 10px 30px rgba(0,0,0,0.35);padding:18px 22px;display:flex;align-items:center;gap:12px;max-width:90%;}
        .global-blocking-spin{width:22px;height:22px;border-radius:50%;border:3px solid rgba(255,255,255,0.25);border-top-color:#60a5fa;animation:gb_spin 0.9s linear infinite}
    ...
    // Expose globally
    window.__showBlocking = showBlocking;
    window.__hideBlocking = hideBlocking;

    // Hook role-select change: chặn thao tác ngay khi user đổi vai
    document.addEventListener('DOMContentLoaded', function(){
        ...
        if (roleSelectEl){
            ...
            roleSelectEl.addEventListener('change', function(){
                if (switching) return;
                switching = true;
                try { roleSelectEl.disabled = true; } catch(e) {}
                showBlocking('Đang chuyển vai trò...');
                // Tự mở khóa phòng trường hợp network treo
                setTimeout(()=>{ switching = false; try{ roleSelectEl.disabled = false; }catch(e){}; hideBlocking(); }, 15000);
            }, { capture: true });
        }
    });

    // Patch fetch để tự hiển thị overlay khi gọi /switch-role
    try {
        const nativeFetch = window.fetch;
        window.fetch = function(url, options){
            const urlStr = typeof url === 'string' ? url : (url?.url || '');
            const isSwitchRole = urlStr.includes('/switch-role');
            if (isSwitchRole){
                showBlocking('Đang chuyển vai trò...');
            }
            return nativeFetch.apply(this, arguments).finally(()=>{
                if (isSwitchRole){
                    // Đợi UI cập nhật xong rồi mới mở khóa cho cảm giác mượt
                    setTimeout(()=>{ hideBlocking(); }, 400);
                }
            });
        };
    } catch(e) { /* ignore */ }
